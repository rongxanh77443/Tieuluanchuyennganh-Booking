...
public class AdminController {
	...
	public String profile(@PathVariable("id") int userid, HttpServletRequest request) {
		request.setAttribute("user", userSv.findById(userid));
		request.setAttribute("profile", managerSv.findByUser(userSv.findById(userid)));
		return "AdminView/profile";
	}

	@GetMapping(value = "/user-list/{id}")
	public String userlist(@PathVariable("id") int userid, HttpServletRequest request) {
		request.setAttribute("user", userSv.findById(userid));
	...
	public String sellerlist(@PathVariable("id") int userid, HttpServletRequest request) {
	...
	public String customerlist(@PathVariable("id") int userid, HttpServletRequest request) {
		request.setAttribute("user", userSv.findById(userid));
	...
	public String carslist(@PathVariable("id") int userid, HttpServletRequest request) {
		request.setAttribute("user", userSv.findById(userid));
	...
	public String routeslist(@PathVariable("id") int userid, HttpServletRequest request) {
		request.setAttribute("user", userSv.findById(userid));
		request.setAttribute("listRoutes", routeSv.findAllRoute());
		return "AdminView/ManagementRoute";
	}

	@GetMapping(value = "/edit/{userid}/{usersid}/{action}")
	public String edituser(@PathVariable("userid") int userid, @PathVariable("usersid") int usersid,
			@PathVariable("action") String action, HttpServletRequest request) {
		System.out.println("Action edit la:   " + action);
		request.setAttribute("user", userSv.findById(userid));
		if (action.equals("user")) {
			request.setAttribute("users", userSv.findById(usersid));
			if ((userSv.findById(usersid).getRoleuserid() == 1 || userSv.findById(usersid).getRoleuserid() == 2)) {
				System.out.println("Edit Manager: " + managerSv.findByUser(userSv.findById(usersid)).getName());
				request.setAttribute("profile", managerSv.findByUser(userSv.findById(usersid)));
				System.out.println(managerSv.findByUser(userSv.findById(usersid)).getName() + "     :Name Manager Choosed");
				request.setAttribute("role", 1);
				if (userSv.findById(usersid).getRoleuserid() == 1)
					request.setAttribute("rolename", "Admin");
				else
					request.setAttribute("rolename", "Seller");
			} else {
				System.out.println("Edit customer: " + customerSv.findByUser(userSv.findById(usersid)).getName());
				request.setAttribute("role", 3);
				request.setAttribute("profile", customerSv.findByUser(userSv.findById(usersid)));
				request.setAttribute("rolename", "Customer");
			}
			System.out.println(userSv.findById(usersid).getRoleuserid() + "     :RoleUserid Choosed");
		}
		if (action.equals("manager")) {
			request.setAttribute("users", userSv.findById(managerSv.findById(usersid).getUser().getId()));
		...
		if (action.equals("customer")) {
			request.setAttribute("users", userSv.findById(customerSv.findById(usersid).getUser().getId()));
			System.out.println("Edit customer: " + customerSv.findById(usersid).getName());
			request.setAttribute("role", 3);
	...
	public String saveuser(@RequestParam int userid, @RequestParam String profileid, @RequestParam String usersid,
			@RequestParam String username, @RequestParam String email, @RequestParam String name,
			@RequestParam String dateofbirth, @RequestParam String gender, @RequestParam String phonenumber,
			@RequestParam String country, @RequestParam String showprofile, @RequestParam String password,
			@RequestParam("roleuser") String roleuser, HttpServletRequest request) {
		request.setAttribute("user", userSv.findById(userid));
		User user = userSv.findById(Integer.parseInt(usersid));
		...
		if (roleuser == "Seller")
			user.setRoleuserid(2);
		if (roleuser == "Customer")
			user.setRoleuserid(3);

		System.out.println(roleuser + "  :roleuser");
		user.setActive(1);
		userSv.saveUser(user);

		if (userSv.findById(Integer.parseInt(usersid)).getRoleuserid() == 1
				|| userSv.findById(Integer.parseInt(usersid)).getRoleuserid() == 2) {
			...
			manager.setShowprofile(Integer.parseInt(showprofile));
			manager.setUser(user);
			managerSv.saveManager(manager);
		} else {
	...
	public String delete(@PathVariable("userid") int userid, @PathVariable("action") String action,
		...
		request.setAttribute("user", userSv.findById(userid));
		if (action.equals("user")) {
			userSv.delete(deleteid);
			System.out.println("Id user sẽ xóa: " + deleteid);
			return "redirect:/user-list/" + userid;
		} else {
			if (action.equals("manager")) {
				if (managerSv.findById(deleteid).getId() != 1) {
					userSv.delete(userSv.findById(managerSv.findById(deleteid).getUser().getId()).getId());
				...
				if (action.equals("customer")) {
					...
					userSv.delete(userSv.findById(customerSv.findById(deleteid).getUser().getId()).getId());
					managerSv.delete(deleteid);
	...
	@PostMapping(value="/savenewmanager")
	public String savenewmanager(@RequestParam("object") String action, @RequestParam("userid") int userid,
			@RequestParam String username, @RequestParam String email, @RequestParam String name,
			@RequestParam String dateofbirth, @RequestParam String gender, @RequestParam String phonenumber,
			@RequestParam String password, HttpServletRequest request) {
		User user =new User(username,email,password,1,2);
		userSv.saveUser(user);
		Manager manager=new Manager(name,dateofbirth,gender,phonenumber,1,user);
		managerSv.saveManager(manager);
		request.setAttribute("user", userSv.findById(userid));
		System.out.println("Action them moi acmin!");
		request.setAttribute("listmanager", managerSv.findAllManager());
		return "redirect:/seller-list/"+userid;
	}
